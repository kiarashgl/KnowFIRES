  ...
  <v-container class="pa-16" fluid>
    <v-row>
      <v-col cols="2">
        ...
        <v-slider min="3" max="15" step="1" thumb-label="always" v-model="topK"></v-slider>
        <v-switch v-model="merged" label="Merged or split"></v-switch>
        <v-switch @change="refreshGraphs" v-model="showNeighbors" label="Show neighbor links"></v-switch>
    ...
    for (const i in ll) {
      ...
      this.plot[gid](document.getElementById(gid))
          ...
          .nodeCanvasObject((node, ctx, globalScale) => {
            ...
            const color = (node.retriever === this.selectedRetrievers[0] ? "#2271B2"
